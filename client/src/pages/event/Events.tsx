    ...
    <div className="min-h-screen bg-space-dark text-white">
      ...
      <main className="container mx-auto px-4 pt-24 pb-16">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="md:w-2/3">
              ...
              <p className="text-gray-300 mb-6">
                Community members can organize their own astronomical events with
              ...
              <ul className="text-gray-400 mb-6 space-y-2">
                ...
                <li className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-space-accent"></div>
                  <span>Guidance from experienced community members</span>
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-space-accent"></div>
                  <span>Promotion through ISA Community channels</span>
                </li>
              </ul>
              <button
        ...
        <section className="mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-8 justify-center text-center">Upcoming Community Events</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {loading ? (
              ...
              (showAll ? events.filter(e => new Date(e.eventDate) > new Date()) : events.filter(e => new Date(e.eventDate) > new Date()).slice(0, 3)).map((event) => (
                <div
                  key={event._id}
          ...
          {events.filter(e => new Date(e.eventDate) > new Date()).length > 3 && !showAll && (
        ...
        {/* Completed ISA Events */}
        {!loading && events.filter(e => new Date(e.eventDate) <= new Date()).length > 0 && (
          <section className="mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-8 justify-center text-center">Completed Community Events</h2>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {events.filter(e => new Date(e.eventDate) <= new Date()).map((event) => (
                <div
                  key={event._id}
                  className="cosmic-card overflow-hidden group relative flex flex-col opacity-80 hover:opacity-100 transition-opacity"
                >
                  <Link
                    to={`/events/${event.slug}`}
                    className="flex-1 flex flex-col"
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      <div>
                        <span
                          className="text-space-purple uppercase text-xs font-bold tracking-widest mb-2"
                        >
                          Completed
                        </span>
                        <h3 className="text-xl font-semibold mb-3">
                        ...
                        <div className="space-y-2 mb-4">
                          <div className="flex items-center text-sm text-gray-400">
                            <Calendar className="h-4 w-4 mr-2 text-space-purple" />
                            <span><FormatDate date={event.eventDate} /></span>
                          </div>
                          <div className="flex items-center text-sm text-gray-400">
                            <Clock className="h-4 w-4 mr-2 text-space-purple" />
                            <span><FormatTime date={event.eventDate} /></span>
                          </div>
                          <div className="flex items-center text-sm text-gray-400">
                            <MapPin className="h-4 w-4 mr-2 text-space-purple" />
                            <span>{event.location}</span>
                          </div>
                        </div>
